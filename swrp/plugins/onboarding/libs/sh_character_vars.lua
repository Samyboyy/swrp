...
ix.char.RegisterVar("careerPath", {
	...
	OnValidate = function(self, value, payload)
		...
		if (!SWRP.IsCareerPath(value)) then
			return false, "chooseCareerTarget"
		end
...
ix.char.RegisterVar("careerTarget", {
	...
	OnValidate = function(self, value, payload)
		...
		if (tostring(payload.careerPath or "") ~= tostring(node.branch or "")) then
			return false, "careerTargetMismatch"
		end
	...
	OnAdjust = function(self, client, data, value, newData)
		...
		if (node) then
			newData.careerTarget = node.id
			newData.careerPath = node.branch
		end
	end
})
